    ...
    <div className="min-h-screen bg-[var(--bg)] flex flex-col">
      ...
      <header className="border-b border-[var(--border)] px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${activeTab === 'generate' ? 'bg-[var(--surface2)] text-[var(--text)]' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}
          >
            Generate
          </button>
          <button
            onClick={() => setActiveTab('library')}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${activeTab === 'library' ? 'bg-[var(--surface2)] text-[var(--text)]' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}
          >
            Library {generations.filter(g => g.saved).length > 0 && <span className="ml-1 text-xs opacity-60">({generations.filter(g => g.saved).length})</span>}
          </button>
        </div>
      </header>
      ...
      {activeTab === 'generate' ? (
        <div className="flex flex-1 overflow-hidden">
          {/* Left: Upload panels */}
          <div className="w-80 border-r border-[var(--border)] overflow-y-auto p-4 space-y-4 shrink-0">
            <UploadPanel
              title="Content Images"
              hint="Photos of the earring to recreate"
              type="content"
              projectId={id!}
            ...
            <UploadPanel
              ...
              hint="Style/composition targets"
              type="reference"
              projectId={id!}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <AssetLibrary
            generations={generations.filter(g => g.saved)}
            onToggleSave={handleToggleSave}
            onDelete={handleDeleteGeneration}
          />
        </div>
