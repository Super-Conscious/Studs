          ...
          <div key={gen.id} className="group relative bg-[var(--surface)] border border-[var(--border)]  overflow-hidden">
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition flex items-end justify-center opacity-0 group-hover:opacity-100">
              <div className="flex gap-2 p-3">
                <button onClick={() => onToggleSave(gen)} className={`px-3 py-1.5  text-xs font-bold transition ${gen.saved ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'bg-white text-black'}`}>
                  {gen.saved ? 'Saved' : 'Save'}
                </button>
                <a href={gen.image_url} download className="px-3 py-1.5 bg-white text-black  text-xs font-bold hover:bg-neutral-100 transition">
                  Download
                </a>
                <button onClick={() => onDelete(gen)} className="px-3 py-1.5 bg-red-500 text-white  text-xs font-bold hover:bg-red-600 transition">
                  Delete
                </button>
              </div>
            </div>
